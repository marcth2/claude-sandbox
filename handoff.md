...
claude-sandbox/
  ...
  profiles/
    omc/
      Dockerfile          ← npm install -g oh-my-claude-sisyphus (the omc npm package)
...
**Convention for adding new profiles:** system-level dependencies (packages, binaries, npm globals) go in the profile's `Dockerfile`; config, skills, and settings go in its `setup.sh`. `omc` gets its own image because `oh-my-claude-sisyphus` is a real npm package (a system dependency); `aihero` doesn't because its skill pack is just Markdown files copied into `.home/.claude/skills/` at setup time (config, not a dependency).
...
**Status: untested.** Only Linux + native Docker Engine has actually been exercised end-to-end (dogfooded across all 3 profiles). The macOS/WSL logic below is real, implemented code — not speculation — but nobody has run it. There's genuine uncertainty specifically around Docker Desktop's virtualization layer (socket permissions, volume mount ownership/performance), which behaves differently enough from native Docker Engine that "the code path exists" isn't the same as "it works." PowerShell/CMD are already rejected outright by the shell guard in `claude.sh`. If you hit problems on macOS or Windows, please file a GitHub issue.
...
All three profiles' `setup.sh` write this common base:
...
**`omc` profile only:** after seeding the base above, `setup.sh` runs `omc setup --quiet` (the
package's own non-interactive CLI setup) via `docker run --entrypoint omc ...`. This merges
`hooks` and a `statusLine` into `settings.json` and merges an OMC content block into `CLAUDE.md` —
generated by the tool itself, not hand-authored here. An earlier version of `setup.sh` hand-seeded
`enabledPlugins`/`extraKnownMarketplaces`/a guessed `statusLine` command; that approach assumed the
marketplace/plugin-install path but this profile actually installs the npm CLI
(`oh-my-claude-sisyphus`), so those keys did nothing. Fixed as part of #13/#15 — see that PR for the
full investigation (including the wrong-npm-package bug: `oh-my-claudecode` on npm is an unrelated
third-party package, not this project's).
